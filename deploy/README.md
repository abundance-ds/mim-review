...
Updates to a Git checkout: pull the chosen commit, run `npm ci --omit=dev`, then restart the service. The hosted instance uses release archives, not a Git checkout: stage `git archive` of the pushed commit in a new directory, install production dependencies using `/opt/node24/bin/npm` with `/opt/node24/bin` first in `PATH`, and record the full commit in `REVISION`. Stop the service, take a SQLite backup of the access database, then move the existing application directory aside and move the staged release to `/opt/mim-review`. Start the service and verify public HTTPS health, private MCP access, conversion, and HTML export. Keep the previous release and a root-only access backup for rollback. Preserve `/etc/mim-review.env`, `/var/lib/mim-review`, and the existing Caddy/systemd configuration. When migrating an older installation, its former data directory and backups are not used or served by this version. Handle those existing files explicitly under the owner's migration policy; this release does not silently delete user files.

To support a browser-based MCP client that supplies an Origin header, set its exact trusted origin in `ALLOWED_ORIGINS`. Arbitrary cross-origin browser access is not enabled. Native clients typically omit Origin. OAuth discovery, registration, and token endpoints support browser CORS. MCP and bearer-authenticated processing endpoints enforce the configured trusted origins. Scoped `/transfer/*` URLs separately permit credential-free cross-origin requests so sandboxed MCP Apps can upload and download; possession, purpose, ownership, expiry, and revocation checks still apply.
